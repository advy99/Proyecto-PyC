...
function comprobarSuma() {
    ...
    for (var i = 0; i <= 5; i++) {
        if ($('.c').eq(i).value == (Number($('.a').eq(i).innerHTML) + Number($('.b').eq(i).innerHTML))) {
            ...
            $('.c').eq(i).style.backgroundColor = "red";
            $('.c').eq(i).style.color = "white";
        }
    }
}
...
function comprobarNumeroMatriz() {
    ...
    for (var i = 0; i <= 5; i++) {
        ...
        if ($('c').eq(i).value == (Number($('a').eq(0).innerHTML) * Number($('b').eq(i).innerHTML))) {
...
function anadirGrafico() {
    ...
    $('#superficies').appendChild(parrafo);
